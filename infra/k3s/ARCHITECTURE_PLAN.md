...
| ops/loki DNS points to ops-1 but Traefik runs on run nodes | Cloudflare LB routes all `*.ml.ink` to run-node pool. Traefik routes to ops-1 services over cluster network. |
...
| registry.internal doesn't resolve in pods                  | In-cluster pods use IP (10.0.1.4:5000). Nodes use /etc/hosts for kubelet image pulls.                  |
...
└────────────────────┬────────────────────────────────────────────┘
   ...
   gRPC outbound    HTTPS                 HTTPS
        │            loki.ml.ink     Cloudflare LB / run pool
        │
        ▼
┌───────────────────────────────────────────────────────────────┐
...
| MCP API (Railway)   | loki.ml.ink        | HTTPS + bearer token  | Logs for `get_service`            |
...
| Temporal Worker     | Registry (ops-1)   | HTTP, 10.0.1.4:5000  | Push images (IP, not DNS)         |
...
| Run nodes (kubelet) | Registry (ops-1)   | HTTP, 10.0.1.4:5000  | Pull images (via registries.yaml) |
...
| ------- | -------------- | ------------------------------------------------------------------ |
| k3s-1   | `k3s-1`        | **Active.** k3s server + Temporal worker.                          |
| ops-1   | `muscle-ops-1` | **Active.** Registry, Loki, Prometheus, Grafana. Coolify still present. |
| build-1 | `builder-1`    | **Active.** BuildKit. Docker CE purged.                            |
| run-1   | `muscle-1`     | **Active.** Traefik + customer containers. Docker CE purged.       |

**Ingress:** Cloudflare LB on `*.ml.ink` routes to run-node origin pool. No direct DNS A records.
...
```
10.0.0.4       k3s-1      (Cloud, auto-assigned)
10.0.0.3       build-1    (Cloud, auto-assigned)
10.0.1.4       ops-1      (Dedicated, static Netplan)
10.0.1.3       run-1      (Dedicated, static Netplan)
10.0.1.101     run-2      (future)
```

Setup:

1. Robot → create vSwitch (VLAN 4000), attach dedicated servers
...
```
*.ml.ink        LB   Cloudflare pool(run-*)   # customer apps
grafana.ml.ink  LB   Cloudflare pool(run-*)   # Grafana
loki.ml.ink     LB   Cloudflare pool(run-*)   # Loki
```

Cloudflare Load Balancer fronts all public hostnames and health-checks run-node origins. Traefik on run nodes then routes to backend services wherever they run. ops-1 never needs direct public web traffic.

### Cloudflare Load Balancing ($5/mo)

All public traffic flows through Cloudflare edge. No direct DNS A records to run nodes.

```
Browser → Cloudflare Edge (terminates TLS) → Origin Pool (run node IPs) → Traefik → k8s Service
```

| Setting            | Value                                          |
| ------------------ | ---------------------------------------------- |
| LB hostname        | `*.ml.ink`                                     |
| Origin pool        | `run-nodes` — list of run node public IPs      |
| SSL mode           | Full (CF terminates edge TLS, HTTPS to origin) |
| Health check       | HTTPS :443 `/`, expect 200+302, interval 60s   |
| Health check host  | `health.ml.ink`                                |
| Failover           | Dead origin removed within ~60s                |
| Bandwidth          | Unlimited (traffic flows through CF edge)      |

**Adding/removing run nodes:** Update the `run-nodes` origin pool in Cloudflare dashboard (Traffic → Load Balancing → Pools). No DNS records to manage.

**Why not Hetzner LB:** ~5TB/mo bandwidth cap, all traffic proxied through a single box. Cloudflare LB is DNS-level steering with unlimited bandwidth through edge PoPs worldwide.

**Why not free Cloudflare proxy:** Free proxy does passive retry-on-failure only — no active health checks, no automatic origin removal. Dead origins cause user-facing errors until manually removed.

### TLS

Wildcard cert in `dp-system`. Traefik uses it as default cert via TLSStore — customer Ingresses don't reference any secret.

**Primary:** Let's Encrypt wildcard cert via cert-manager DNS-01 (auto-renews every 90 days).

**Backup:** Cloudflare Origin CA cert for `*.ml.ink` (15-year validity, trusted only by CF edge). Stored in Ansible vault as `cloudflare_origin_cert` / `cloudflare_origin_key`. Can replace the Let's Encrypt cert if cert-manager has issues.
...
```
create_service(name="my-app", ...)   →  https://my-app.ml.ink
create_service(name="cool-api", ...) →  https://cool-api.ml.ink
```
...
3. User adds CNAME: `mycoolapp.com → my-app.ml.ink`
...
```
*.ml.ink / grafana.ml.ink / loki.ml.ink
      → Cloudflare Load Balancer pool (run node public IPs)
      → Traefik (hostNetwork on run nodes)
      → k8s Service
      → Pod (any node per selector)
```
...
MCP API (Railway) → Temporal Cloud → Worker (k3s-1) picks up task
  ...
  │    buildctl build:
  │      --import-cache type=registry,ref=10.0.1.4:5000/cache/<service>:latest
  │      --export-cache type=registry,ref=10.0.1.4:5000/cache/<service>:latest,mode=max
  ...
  │
  └─ Return: {status: "running", url: "my-app.ml.ink", commit_sha: "abc123"}
```
...
```bash
docker build -f go-backend/k8s-worker.Dockerfile -t registry.internal:5000/dp-system/temporal-worker:latest go-backend
...
```bash
iptables -A INPUT -p tcp --dport 5000 -s 10.0.0.0/16 -j ACCEPT
...
MCP API on Railway queries: `https://deploy:<password>@loki.ml.ink/loki/api/v1/query_range?query=...`
...
```
# Runtime logs
GET https://deploy:<pw>@loki.ml.ink/loki/api/v1/query_range
...
- hosts: "{{ target | default(ansible_limit) }}"
  ...
  post_tasks:
    ...
    - name: Reminder
      debug:
        msg: "{{ inventory_hostname }} is Ready. Add {{ ansible_host }} to Cloudflare origin pool 'run-nodes'."
```
...
| Cloudflare LB     | CF dashboard shows origin pool healthy (green)  |
| Traefik + TLS     | `curl https://test.ml.ink` via CF edge          |
| gVisor            | Customer pod: `dmesg \| grep gVisor`                |
| Temporal worker   | Connects to Temporal Cloud, completes test workflow |
| Loki auth         | `curl https://loki.ml.ink/ready` with auth    |
| Grafana           | `grafana.ml.ink` shows all node health        |
| Egress blocked    | Customer pod can't reach 10.0.1.4:5000             |
...
| Add run node      | Playbook → Ready in < 15 min, CF pool updated       |
...
8. Gitea on ops-1
9. Automate CF origin pool updates via Cloudflare API in add-run-node playbook
